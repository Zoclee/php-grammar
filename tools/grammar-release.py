...
commands = [
    ...
    ('explicit-ast', p + ['tools/php85-ast-conformance.php']),
    ('systematic-structure', p + ['tools/php85-systematic-structure.php']),
    ('declaration-folding', p + ['tools/php85-boundary-folding.php', php]),
    ('phase6-matrices', composer + ['conformance:phase6']),
    ('phase6-matrix-freshness', p + ['tools/php85-phase6.php', '--check']),
    ('scanner-product', p + ['tools/php85-scanner-product.php']),
    ('lexer-short-enabled', p + ['-d', 'short_open_tag=1', 'tools/php85-lexer-differential.php']),
    ('lexer-short-disabled', p + ['-d', 'short_open_tag=0', 'tools/php85-lexer-differential.php']),
    ('positive-report-freshness', p + ['tools/php85-coverage-report.php', '--check']),
    ('negative-report-freshness', p + ['tools/php85-negative-report.php', '--check']),
    ('compiler-source-hashes', py + ['tools/php85-compiler-boundaries.py', str(args.source_directory), '--check']),
    ('parser-reconciliation', py + ['tools/php85-reconcile.py', str(args.source_directory), '--check']),
    ('phase6-evidence-freshness', py + ['tools/php85-phase6-evidence.py', '--check']),
    ('interpolation-binding', p + ['tools/php85-interpolation-binding.php', '--check']),
    ('diagnostic-predicates', p + ['tools/php85-diagnostic-witnesses.php', '--check']),
    ('source-correspondence', py + ['tools/php85-source-correspondence.py', '--cache', str(args.source_cache), '--check']),
    ('final-certification', py + ['tools/php85-certification.py', '--source-cache', str(args.source_cache), '--check']),
